...
subtitle: "Conférence Gestion des données pour le CEISM"
...
format:
  revealjs:
    ...
    footer: "[{{< fa house >}} https://pmassicotte.github.io/cesium_2025](https://pmassicotte.github.io/cesium_2025)"
    height: 1080
    width: 1920
    slide-number: c/t
    transition: fade
...
citation:
  url: https://pmassicotte.github.io/cesium_2025/
---
...
In social sciences, researchers often collect or utilize spatial data to analyze patterns, relationships, and distributions across geographic areas.
...
4. **Community resilience**: Locations of support networks or programs.

5. **Demographics**: Age, income, or education distribution linked to mental health.
...
It is usually a better idea to work with spatial objects rather than tabular data.
...
ggplot() +
  geom_sf(data = quebec, color = "black", fill = NA) +
...
For example, _find all the health and social services network establishments_ within 5 km of **Vanier**.
...
# Calculate centroids
centroids <- st_centroid(quebec)

# Select Vanier and calculate its centroid
vanier <- quebec |>
  filter(NOM == "Vanier") |>
  st_centroid()

# Create a distance matrix
dist_matrix <- st_distance(pts, vanier)
...
ggplot() +
  ...
  geom_sf(
    data = st_buffer(vanier, dist = threshold),
    fill = NA,
    color = "blue",
    linetype = 2L
  ) +
  geom_sf(data = connections_sf, color = "blue", linewidth = 0.25) +
  theme_minimal() +
  theme(
    panel.background = element_rect(fill = "transparent", color = NA),
    axis.title = element_blank(),
...
url <- pins::board_url(
  list(
    seanoe = "https://www.seanoe.org/html/stat/2022/uj4mfvvquq-philippemassicottetakuvikulavalca.xls"
  )
)
...
df_viz <- df |>
  ...
  mutate(dataset_title = str_wrap(dataset_title, 55L)) |>
